...
export default function EditJobCardPage() {
  ...
  return <PageContainer title="Edit Job Card" description="Update service job information" actions={<Link to="/job-cards"><Button variant="outline" icon={<ArrowLeft className="h-4 w-4" />}>Back to Job Cards</Button></Link>}>{isLoading ? <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">Loading job card...</div> : null}{isError || !jobCard ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-8 text-center text-sm text-rose-700">Unable to load this job card.</div> : null}{!isLoading && !isError && jobCard?.status === 'CANCELLED' ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center text-sm text-amber-800">Cancelled job cards cannot be updated.</div> : null}{!isLoading && !isError && jobCard && jobCard.status !== 'CANCELLED' ? <JobCardForm defaultValues={defaultValues} submitLabel="Save Changes" submittingLabel="Saving..." isSubmitting={mutation.isPending} onSubmit={handleUpdate} /> : null}</PageContainer>
}
